...
//            echo "</pre>";
        foreach( $items as $item) {
            ...
            $description_p_horizontal   = ( $type == "horizontal" ) ? '<p>'. esc_html($description) .'</p>' : '';

            $description_p_vertical     = ( $type == "vertical" ) ? '<p>'. esc_html($description) .'</p>' : '';
            ...
            if( isset($item['is_icon']) && $item['is_icon'] == 'custom' ){
                $is_icon =  '<img class="pba-img" src="'.$image[0] .'" alt="'.$image_alt.'" />';
            }
            if( 'vertical' == $type ){
                $button = "";
            }
